...
pub fn main() {
    ...
    app.run(move |cx| {
        ...
        agent_ui::init(
            ...
            init_chat_history(app_state.clone(), cx);
	...
	// Initialize chat history persistence and spawn legacy replay.
	fn init_chat_history(app_state: Arc<AppState>, cx: &mut App) {
	    ...
	    if let Err(e) = std::fs::create_dir_all(&chat_history_dir) {
	        ::log::error!("chat_history: create_dir_all failed: {e}");
	        return;
	    }
	    ::log::info!("chat_history: dir ready {:?}", chat_history_dir);

	    // Pre-create DB file
	    let db_path = chat_history_dir.join("chat_history.db");
	    if !db_path.exists() {
	        match std::fs::OpenOptions::new().create(true).write(true).open(&db_path) {
	            Ok(_) => ::log::info!("chat_history: pre-created db file {:?}", db_path),
	            Err(e) => {
	                ::log::error!("chat_history: pre-create failed: {e}");
	                return;
	            }
	        }
	    }
	    ...
	    // Run migrations
	    let mig_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
	        .parent()
	        .expect("parent of zed crate")
	        .join("chat_history_tools")
	        .join("migrations");
	    ::log::info!("chat_history: migrations dir {:?}", mig_dir);
	    ...
	    let chat_history_handles = match handle.block_on(init_chat_history_tools_async(options)) {
	        Ok(h) => h,
	        Err(e) => {
	    ...
	    acp_thread::chat_history_integration::init(chat_history_handles.tools.clone());
	    let guard = handle.block_on(async { chat_history_handles.store.lock().await });
	    ::log::info!(
	        "chat_history: initialized (embedding_model={}, persistence=enabled)",
	        guard.config().embedding_model
	    );

	    // Enable tool in existing profiles
	    ...
	    // Legacy replay task
	    cx.spawn({
	        let tools = chat_history_handles.tools.clone();
	        async move |cx| {
	            use serde_json::json;
	            use agent::thread_store::{
	                legacy_threads_metadata, legacy_load_thread, SerializedMessageSegment,
	            };
	            use language_model::Role;

	            let sentinel = paths::database_dir()
	                .join("chat_history")
	                .join(".legacy_replay_done");
	            if sentinel.exists() {
	                ::log::info!("legacy_replay: sentinel present, skipping");
	                return;
	            }
	            ::log::info!("legacy_replay: starting");

	            let raw_list = tools.chat_list(r#"{"limit":1000}"#).await;
	            let parsed: serde_json::Value =
	                serde_json::from_str(&raw_list).unwrap_or_else(|_| json!({ "ok": false }));
	            let chats = parsed
	                .get("chats")
	            ...
	            let mut by_title: HashMap<String, serde_json::Value> = HashMap::new();
	            for c in &chats {
	                if let Some(title) = c.get("title").and_then(|t| t.as_str()) {
	                    let has_project = c.get("project_id").and_then(|p| p.as_str()).is_some();
	                    match by_title.get(title) {
	                        None => {
	                            by_title.insert(title.to_string(), c.clone());
	                        }
	                        Some(existing) => {
	                            let existing_has_project =
	                                existing.get("project_id").and_then(|p| p.as_str()).is_some();
	                            if has_project && !existing_has_project {
	                                by_title.insert(title.to_string(), c.clone());
	                            }
	                        }
	                    }
	            ...
	            let needs_replay = chats
	                .iter()
	                .any(|c| c.get("total_messages").and_then(|m| m.as_u64()).unwrap_or(0) == 0);
	            if !needs_replay {
	                let _ = std::fs::write(&sentinel, b"no_replay");
	                ::log::info!("legacy_replay: nothing to replay");
	                return;
	            }

	            // Load legacy thread metadata
	                    let meta_task = cx.update(|cx| legacy_threads_metadata(cx)).ok();
	                    let meta_list = if let Some(task) = meta_task {
	            ...
	            for meta in meta_list.iter().rev() {
	                let title = meta.summary.to_string();
	                if let Some(existing) = by_title.get(&title) {
	                    if existing
	                        .get("total_messages")
	                        .and_then(|m| m.as_u64())
	                        .unwrap_or(0)
	                    ...
	                    let load_task = cx.update(|cx| legacy_load_thread(cx, meta.id.clone())).ok();
	                    let serialized_val = if let Some(task) = load_task {
	                        match task.await {
	                            Ok(Some(s)) => s,
	                            Ok(None) => continue,
	                            Err(_) => continue,
	                ...
	                let project_id = serialized_val
	                    .initial_project_snapshot
	                    .as_ref()
	                    .and_then(|snap| snap.worktree_snapshots.first())
	                    .and_then(|ws| {
	                ...
	                let mut first = true;
	                    ...
	                    for message in &serialized_val.messages {
	            ...
	            let summary =
	                format!("threads={},messages={}", imported_threads, imported_messages);
	            let _ = std::fs::write(&sentinel, summary.as_bytes());
	            ::log::info!("legacy_replay: completed {}", summary);
	        }
	    }).detach();
	}
